...
communication. It consists of a small MatsSocketServer API which is implemented on top of the _Mats<sup>3</sup> API_
and _JSR 356 Java API for WebSockets_ (which most Servlet Containers implement), as well as client libraries - for which
...
From the server, you may request and require the client to renew authentication. You may also ask the client to
reconnect, which is done automatically when the MatsSocketServer is taken down, so that the client hops off and connects
to another server - seamlessly continuing the session after re-auth. You may also close the session, forcing the client
